...
function DashboardApp({ user, onLogout }){
  ...
  useEffect(()=>{
    ...
    if(!initialSignal.current && notif && "Notification" in window && Notification.permission==="granted"){
      new Notification(`SNIPER XAUUSD ${sig.status} — ${result}`,{
...
function AppShell(){
  ...
  if(auth==="loading") return <div className="auth-loading">Loading SNIPER XAUUSD…</div>;
