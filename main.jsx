...
function DashboardApp({ user, onLogout }){
  ...
  const page={
    ...
    History:<section className="page-stack"><div className="page-intro"><div><span className="muted">SIGNAL LOG</span><h2>Signal History</h2><p>Completed signals and TP/SL results only.</p></div><div className="history-count"><b>{trades.filter(t=>["TP2 HIT","SL HIT"].includes(String(t.status).toUpperCase())).length}</b><span>completed</span></div></div><div className="history-section"><div className="history-section-title"><span className="section-dot done-dot"></span><div><b>COMPLETED SIGNALS</b><small>Finished trades with TP or SL result</small></div></div>{trades.filter(t=>["TP2 HIT","SL HIT"].includes(String(t.status).toUpperCase())).length?<div className="history-cards">{trades.filter(t=>["TP2 HIT","SL HIT"].includes(String(t.status).toUpperCase())).map(t=><TradeTimeline key={t.id} trade={t}/>)}</div>:<div className="panel empty-page"><HistoryIcon size={34}/><b>No completed signals yet</b><span>Completed TP/SL results will appear here automatically.</span></div>}</div></section>,
