...
function SignalHistoryCard({ x, live = true }) {
  const result = x.result || x.outcome || "MONITORING";
  const isSL = result === "SL HIT";
  const tp1 = result === "TP1 HIT" || result === "TP2 HIT" || result === "TP3 HIT";
  const tp2 = result === "TP2 HIT" || result === "TP3 HIT";
  const tp3 = result === "TP3 HIT";
  const done = isSL || tp3;
  return <div className={"history-signal-card " + (isSL ? "history-loss" : done ? "history-win" : "history-live")}>
    <div className="history-signal-head">
      <div><span className={"table-pill " + x.status.toLowerCase()}>{x.status}</span><b>XAUUSD · {x.timeframe}</b></div>
      <small>{new Date(x.time).toLocaleTimeString()}</small>
    </div>
    <div className="history-price-grid">
      <Metric label="ENTRY" value={fmt(x.entry,2)}/>
      <Metric label="STOP LOSS" value={fmt(x.sl,2)}/>
      <Metric label="TP1" value={fmt(x.tp1,2)}/>
      <Metric label="TP2" value={fmt(x.tp2,2)}/>
    </div>
    <div className="history-timeline">
      <div className="history-step done"><i>✓</i><span>ENTRY</span></div>
      <div className="history-line done"/>
      <div className={"history-step " + (isSL ? "fail" : "done")}><i>{isSL ? "✕" : "✓"}</i><span>{isSL ? "SL HIT" : "MONITORING"}</span></div>
      <div className="history-line " + (tp1 ? "done" : "")/>
      <div className={"history-step " + (tp1 ? "done" : "pending")}><i>{tp1 ? "✓" : "○"}</i><span>{tp1 ? "TP1 HIT" : "TP1"}</span></div>
      <div className="history-line " + (tp2 ? "done" : "")/>
      <div className={"history-step " + (tp2 ? "done" : "pending")}><i>{tp2 ? "✓" : "○"}</i><span>{tp2 ? "TP2 HIT" : "TP2"}</span></div>
    </div>
    <div className="history-result"><span>{live && !done ? "🟡 MONITORING" : isSL ? "🔴 SL HIT" : "🟢 " + result}</span><small>{live && !done ? "Waiting for price to hit TP or SL" : "Signal completed"}</small></div>
  </div>;
}
...
function DashboardApp({ user, onLogout }){
  ...
  const page={
    ...
    History:<section className="page-stack"><div className="page-intro"><div><span className="muted">SIGNAL LOG</span><h2>Signal History</h2><p>One signal per card. Live signals stay on top; completed signals will move below.</p></div><div className="history-count"><b>{history.length}</b><span>records</span></div></div>{history.length?<><div className="history-section-label"><span>LIVE / CURRENT SIGNAL</span><small>Tracking the latest signal</small></div><SignalHistoryCard x={history[0]} live/><div className="history-section-label"><span>COMPLETED SIGNAL HISTORY</span><small>Finished signals will appear here</small></div><div className="panel empty-page history-completed-empty"><HistoryIcon size={28}/><b>No completed signals yet</b><span>When TP or SL is hit, the signal will move here automatically.</span></div></>:<div className="panel empty-page"><HistoryIcon size={34}/><b>No confirmed signals recorded yet</b><span>SNIPER will record a new entry when a timeframe confirms a setup.</span></div>}</section>,
