...
function DashboardApp({ user, onLogout }){
  ...
  const page={
    ...
    History:<section className="page-stack"><div className="page-intro"><div><span className="muted">SIGNAL LOG</span><h2>Signal History</h2><p>One signal per card. Active signals stay above; completed signals move below.</p></div><div className="history-count"><b>{trades.length}</b><span>signals</span></div></div>{trades.length?<>
      <section className="history-section"><div className="history-section-head"><div><span className="muted">LIVE</span><h3>Current Signal</h3></div><span>Monitoring</span></div><div className="history-cards">{trades.filter(x=>!["TP2 HIT","SL HIT"].includes(String(x.status).toUpperCase())).slice(0,1).map(x=><TradeTimeline trade={x} key={x.id}/>)}{!trades.some(x=>!["TP2 HIT","SL HIT"].includes(String(x.status).toUpperCase()))&&<div className="panel empty-page"><HistoryIcon size={28}/><b>No active signal</b><span>New confirmed signals will appear here.</span></div>}</div></section>
      <section className="history-section"><div className="history-section-head"><div><span className="muted">ARCHIVE</span><h3>Completed Signal History</h3></div><span>{trades.filter(x=>["TP2 HIT","SL HIT"].includes(String(x.status).toUpperCase())).length} completed</span></div><div className="history-cards">{trades.filter(x=>["TP2 HIT","SL HIT"].includes(String(x.status).toUpperCase())).slice(0,20).map(x=><TradeTimeline trade={x} key={x.id}/>)}{!trades.some(x=>["TP2 HIT","SL HIT"].includes(String(x.status).toUpperCase()))&&<div className="panel empty-page"><HistoryIcon size={28}/><b>No completed signals yet</b><span>When TP2 or SL is hit, the signal will move here automatically.</span></div>}</div></section>
    </>:<div className="panel empty-page"><HistoryIcon size={34}/><b>No monitored signals yet</b><span>A confirmed BUY/SELL will be stored automatically for TP/SL monitoring.</span></div>}</section>,
