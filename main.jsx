...
function DashboardApp({ user, onLogout }){
  ...
  const page={
    ...
    History:<section className="page-stack"><div className="page-intro"><div><span className="muted">SIGNAL LOG</span><h2>Signal History</h2><p>Only confirmed BUY/SELL entries are recorded.</p></div><div className="history-count"><b>{history.length}</b><span>records</span></div></div><div className="panel table-panel">{trades.length?<table><thead><tr><th>TIME</th><th>TF</th><th>DIRECTION</th><th>STATUS</th><th>ENTRY</th><th>SL</th><th>TP1</th><th>TP2</th><th>LAST</th></tr></thead><tbody>{trades.map(x=><tr key={x.id}><td>{new Date(x.signal_time||x.created_at).toLocaleTimeString()}</td><td>{x.timeframe}</td><td><span className={"table-pill "+String(x.direction).toLowerCase()}>{x.direction}</span></td><td><span className={"table-pill "+(String(x.status).includes("TP")?"buy":String(x.status).includes("SL")?"sell":"")}>{x.status}</span></td><td>{fmt(x.entry,2)}</td><td>{fmt(x.sl,2)}</td><td>{fmt(x.tp1,2)}</td><td>{fmt(x.tp2,2)}</td><td>{fmt(x.last_price,2)}</td></tr>)}</tbody></table>:<div className="empty-page"><HistoryIcon size={34}/><b>No monitored signals yet</b><span>A confirmed BUY/SELL will be stored automatically for TP/SL monitoring.</span></div>}</div></section>,
